...
Otess.utils = {
	isUrl:function (str_url) { 
		var strRegex = "^((https|http|rtmp|ftp|rtsp|mms)?://)[^\s]*";     
	...
	initForm : function(options) {
		var select = "",lineheight_select="";
		for (var i = 8; i <= 100; i++) {
			select += "<option value='" + i + "' >" + i + "</option>";
		}
		for(var i=13;i<=105;i++){
			lineheight_select += "<option value='" + i + "' >" + i + "</option>";
		}
		var fontsize=$("[name='fontsize']"),family=$("[name='family']"),fonttype=$("[name='fonttype']"),
		align=$("[name='align']"),valign=$("[name='valign']"),timetype=$("[name='timetype']"),
		datatype=$("[name='datatype']"),linetype=$("[name='linetype']"),area=$("[name='area']"),speed=$("[name='speed']"),
		scrolltype=$("[name='scrolltype']"),lineheight=$("[name='lineheight']");
		
		fontsize.empty().append(select).val(12);
		lineheight.empty().append(lineheight_select).val(17);
		fontsize.on("change",function(){
			lineheight.val(Number($(this).val())+5);
		})
		//仿宋、黑体、华文彩云、华文细黑、华文新魏、华文行楷、华文中宋、楷体、隶书、幼圆
		family.empty().append(
				"<option value='微软雅黑'>微软雅黑</option>").append(
				"<option value='黑体'>黑体</option>").append(
				"<option value='仿宋'>仿宋</option>").append(
				"<option value='华文彩云'>华文彩云</option>").append(
				"<option value='华文细黑'>华文细黑</option>").append(
				"<option value='华文新魏'>华文新魏</option>").append(
				"<option value='华文行楷'>华文行楷</option>").append(
				"<option value='华文中宋'>华文中宋</option>").append(
				"<option value='楷体'>楷体</option>").append(
				"<option value='隶书'>隶书</option>").append(
				"<option value='幼圆'>幼圆</option>");
		fonttype.empty().append(
				"<option value='normal'>正常</option>").append(
				"<option value='bold'>粗体</option>").append(
				"<option value='italic'>斜体</option>");
		align.empty().append(
				"<option value='center'>居中</option>").append(
				"<option value='left'>靠左</option>").append(
				"<option value='right'>靠右</option>");
		valign.empty().append(
			"<option value='top'>顶部</option>").append(
			"<option value='middle'>中间</option>").append(
			"<option value='bottom'>底部</option>");
		timetype.empty().append(
				"<option value='1'>日期+星期+时间</option>").append(
				"<option value='2'>日期+时间</option>").append(
				"<option value='3'>星期+时间</option>").append(
				"<option value='4'>日期</option>").append(
				"<option value='5'>星期</option>").append(
				"<option value='6'>时间</option>");
		datatype.empty().append(
		"<option value='0'>今日天气</option>").append(
		"<option value='1'>明日天气</option>");
		linetype.empty().append("<option value='1'>多行</option>").append(
						"<option value='2'>单行</option>");
		$("[name='area']").empty().append(
				"<option value='1,08,中国'>中国</option>").append(
				"<option value='0,12,国际换日线'>国际换日线</option>").append(
				"<option value='0,11,夏威夷'>夏威夷</option>").append(
				"<option value='0,09,太平洋'>太平洋</option>").append(
				"<option value='0,07,美加山区'>美加山区</option>").append(
				"<option value='0,06,南美洲太平洋'>南美洲太平洋</option>").append(
				"<option value='0,05,美东'>美东</option>").append(
				"<option value='0,04,大西洋'>大西洋</option>");
		speed.empty().append("<option value='0'>不滚动</option>")
			.append("<option value='1'>慢速</option>")
			.append("<option value='2'>中速</option>")
			.append("<option value='3'>快速</option>");
		scrolltype.empty()
			.append("<option value='left'>向左</option>")
			.append("<option value='up'>向上</option>")
		if(options){
			if(options.tf_style_web){
				var style = options.tf_style_web.split('|');
				$("[name='color']").val(style[0]);
				$("[name='backgroundcolor']").val(style[1]);
				fontsize.val(style[2]);
				family.val(style[3]);
				fonttype.val(style[4]);
				align.val(style[7]);
				valign.val(style[8]);
				speed.val(style[6]);
				scrolltype.val(style[5]);
				lineheight.val(style[9]);
			}
		}
	},
	getDuration : function(name) {
		...
		if(name){
			hour=name+hour,minute=name+minute,second=name+second
		}
		console.log(hour+","+minute+","+second);
		var h = parseInt($("[name='"+hour+"']").val()), m = parseInt($("[name='"+minute+"']").val())
			, s = parseInt($("[name='"+second+"']").val());
		console.log(h,m,s);
		return h * 60 * 60 + m * 60 + s;
	},
	getTimeToDuration:function(time){
		var h=time.split(":")[0],m=time.split(":")[1];
		return Number(h)*60*60+Number(m)*60;
	},
	setDuration:function(name,druation){
		var time=Otess.utils.getDurationArray(druation);
		var hour="hour" ,minute="minute",second="second";
		if(name){
			hour=name+hour,minute=name+minute,second=name+second
		}
		$("[name='"+hour+"']").val(time[0]);
	    $("[name='"+minute+"']").val(time[1]);
	    $("[name='"+second+"']").val(time[2]);
	},
	...
	getDurationArray : function(Duration) {
		var nH = 0;
		var nM = 0;
		var nS = 0;
		nDuration = parseInt(Duration);
		if (nDuration >= 60 * 60) {
			nH = parseInt(nDuration / (60 * 60));
			nDuration -= parseInt(nH) * 60 * 60;
		}
		if (nDuration >= 60) {
			nM = parseInt(nDuration / 60);
			nDuration -= parseInt(nM) * 60;
		}
		nS = nDuration;
		nHH = nH < 10 ? "0" + nH : nH;
		nMM = nM < 10 ? "0" + nM : nM;
	...
	renderSize : function(size) {
		...
		if (size < 1024) {
			...
			filesize = (size / (1024 * 1024 * 1024)).toFixed(2) + "MB";
		}
		return filesize;
	},
	renderFileType : function(type) {
		//1=文本 2=网页 3=图片 4=音频 5=视频
		var fileType = "<span class=\"label label-default\">文本</span>";
		switch (type) {
			case 1:
				fileType = "<span class=\"label label-default\">文本</span>";
				break;
			case 2:
				fileType = "<span class=\"label label-primary\">网页</span>";
				break;
			case 3:
				fileType = "<span class=\"label label-info\">图片</span>";
				break;
			case 4:
				fileType = "<span class=\"label label-danger\">音频</span>";
				break;
			case 5:
				fileType = "<span class=\"label label-warning\">视频</span>";
				break;
			case 6:
				fileType = "<span class=\"label label-warning\">PDF</span>";
				break;
			case 7:
				fileType = "<span class=\"label label-warning\">直播</span>";
				break;
		}
		return fileType;
	}
...
Otess.utils.city={
	init:function(_cmbProvince, _cmbCity, _cmbArea, defaultProvince, defaultCity, defaultArea){
		$("#" + _cmbProvince).empty(); $("#" + _cmbCity).empty(); $("#" + _cmbArea).empty();
	    ...
	    var citys=Otess.utils.city.prov();
	    for (var i = 0; i < citys.length; i++) {
	        cmbAddOption(cmbProvince, citys[i].name, citys[i]);
	    }
